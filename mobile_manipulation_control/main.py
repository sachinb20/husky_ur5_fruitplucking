...
class Robot:
    ...
    def ComputeTrajectory(self):
        ...
        # the below lines of code ensure that the robot returns to its initial configuration
        # #my addition start here
        # #Tse_final_grasp to Tse_final_standoff
        # trajectories.append([self.traj_gen.Generate_segment_trajectory(Tse_final_grasp, Tse_final_standoff, self.time_scale, self.traj_type),self.gripper_open])

        # # Tse_final_grasp to Tse_inital_pos
        # Tse_final_standoff = np.dot(self.Tsc_final,self.Tce_standoff)
        # trajectories.append([self.traj_gen.Generate_segment_trajectory(Tse_final_standoff, self.Tse_init, self.time_scale, self.traj_type), self.gripper_closed])
